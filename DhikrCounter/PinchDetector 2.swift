...
public final class PinchDetector {
    ...
    func process(frames: [SensorFrame]) -> [PinchEvent] {
        ...
        // Log individual peaks
        for (index, candidate) in peakCandidates.enumerated() {
            let snr = candidate.value / (fusedMean + fusedStd + 0.001) // Avoid division by zero
            debugLog("📍 Peak \(index + 1): t=\(String(format: "%.2f", t[candidate.index]))s, val=\(String(format: "%.6f", candidate.value)), SNR=\(String(format: "%.1f", snr))")
        }
        ...
        for candidate in peakCandidates {
            let preIdx = Int((config.windowPreMs / 1000.0) * fs)
            let postIdx = Int((config.windowPostMs / 1000.0) * fs)
            
            let startIdx = max(0, candidate.index - preIdx)
            let endIdx = min(fusedSignal.count - 1, candidate.index + postIdx)
            
            guard endIdx > startIdx else { continue }
            
            let window = Array(fusedSignal[startIdx...endIdx])
            ...
            if bestNccScore >= config.nccThresh {
                ...
                debugLog("✅ Peak \(templateMatchCount) PASSED: Template \(bestTemplateIndex + 1) NCC=\(String(format: "%.3f", bestNccScore)), confidence=\(String(format: "%.3f", confidence))")
            } else {
                debugLog("❌ Peak FAILED: Best template NCC=\(String(format: "%.3f", bestNccScore)) < \(String(format: "%.2f", config.nccThresh)) (tried \(templates.count) templates)")
            }
        }
